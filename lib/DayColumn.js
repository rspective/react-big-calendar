...
DayColumn.propTypes = {
  ...
  iconAccessor: _propTypes3.accessor,
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.renderEvents = function() {
    var _props2 = _this2.props,
      ...
      iconAccessor = _props2.iconAccessor,
    ...
    return styledEvents.map(function(_ref2, idx) {
      ...
      var icon = (0, _accessors.accessor)(event, iconAccessor)
          ...
          _react2.default.createElement(
            ...
            [
              icon &&
                _react2.default.createElement(
                  'i',
                  { className: 'material-icons', key: icon + idx },
                  icon
                ),
              label,
            ]
  ...
  this._selectable = function() {
    ...
    var selectionState = function selectionState(_ref3) {
      ...
      if (!_this2.state.selecting) {
        ...
        _this2.startedOnHourAfterTheRemovedOne = undefined
      }

      var initial = _this2._initialDateSlot

      // snap to slot fix for DST changes
      var minOffset = new Date(min).getTimezoneOffset()
      var initialOffset = new Date(initial).getTimezoneOffset()
      var currentOffset = new Date(current).getTimezoneOffset()
      var isFromSummerToWinter =
        minOffset !== currentOffset && new Date(min).getMonth() === 9
      debugger // eslint-disable-line
      if (minOffset !== currentOffset) {
        var hourAfterTheRemovedOne = 3 // 3AM right after 1 AM
        var distanceFromTopChangeDST = 1 / 23 * hourAfterTheRemovedOne
        var distanceFromTop = (y - top) / range
        // winter -> summer
        if (initialOffset < minOffset) {
          ...
          if (distanceFromTop > distanceFromTopChangeDST) {
            ...
            if (!_this2.startedOnHourAfterTheRemovedOne) {
              initial = _dates2.default.add(
                initial,
                isFromSummerToWinter ? -60 : 60,
                'minutes'
              )
            }
            current = _dates2.default.add(
              current,
              isFromSummerToWinter ? -60 : 60,
              'minutes'
            )
            ...
            if (!_this2.state.selecting)
              _this2.startedOnHourAfterTheRemovedOne = true
            if (!_this2.startedOnHourAfterTheRemovedOne) {
              initial = _dates2.default.add(
                initial,
                isFromSummerToWinter ? -60 : 60,
  ...
  this._selectSlot = function(_ref4) {
    ...
    var current = startDate,
      repeatLimit = 24 * 60 / _this2.props.step,
      slots = []

    while (_dates2.default.lte(current, endDate) && repeatLimit) {
      repeatLimit--
      slots.push(current)
